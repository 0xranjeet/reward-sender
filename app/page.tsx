...
export default function Home() {
  ...
  async function detectContractChain(addressToCheck: Address) {
    const prioritizedChains = [...supportedChains].sort((left, right) => {
      if (left.id === chainId) {
        return -1;
      }
    ...
    const matches = await Promise.all(
      prioritizedChains.map(async (chain) => {
        ...
        try {
          const bytecode = await client.getBytecode({ address: addressToCheck });
          return bytecode && bytecode !== "0x" ? chain.id : null;
        } catch {
          // Ignore flaky RPCs on unrelated chains so detection can continue.
          return null;
        }
      })
    );
